...
class QM9MetricsCalculator:
    ...
    def __init__(self, input_dir: Path, output_dir: Path):
        ...
        # self.default_train_smiles = Path("/data/qinyifei/model/Diff-3D/data/data_reference/QM9/QM9_smiles.txt")
        # self.default_training_set = Path("/data/qinyifei/model/Diff-3D/data/training_set_reference/QM9/smiles.smi")
        # self.default_fpscores =Path("/data/qinyifei/model/Diff-3D/data/fpscores.pkl.gz")
        self.default_train_smiles = PROJECT_ROOT / "data/data_reference/QM9/QM9_smiles.txt"
        self.default_training_set = PROJECT_ROOT / "data/training_set_reference/QM9/smiles.smi"
        self.default_fpscores = PROJECT_ROOT / "data/fpscores.pkl.gz"
        print(self.default_train_smiles)
        print(self.default_training_set)
        print(self.default_fpscores)
        # 从config获取或使用默认路径
    ...
    # 验证训练集文件是否可读
        try:
            with open(self.train_smiles_file, 'r') as f:
                pass  # 只是测试文件是否可打开
        except IOError as e:
            raise IOError(f"Cannot read training smiles file: {self.train_smiles_file}. Error: {str(e)}")
        validity_connect(self.input_dir, self.output_dir)
    ...
    def run_all_metrics(self) -> None:
        ...
        try:
            ...
            print("QM9 metrics processing completed successfully.")
        except Exception as e:
            print(f"Error in QM9 metrics processing: {str(e)}")
            raise

def qm9_metrics(input_directory: str, output_directory: str) -> None:
    """QM9指标计算入口函数"""
