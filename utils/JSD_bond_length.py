...
def calculate_JSD_bond_length(base_path, dataset):
    """Calculate JS divergence for bond lengths"""
    # Define file pairs to compare
    file_pairs = [
        ('C-C-optimized.txt', 'C-C.txt'),
        ('C-N-optimized.txt', 'C-N.txt'),
        ('C-O-optimized.txt', 'C-O.txt'),
        ('C=C-optimized.txt', 'C=C.txt'),
        ('C=N-optimized.txt', 'C=N.txt'),
        ('C=O-optimized.txt', 'C=O.txt'),
        ('C#C-optimized.txt', 'C#C.txt'),
    ...
    # Set paths
    input_dir = os.path.join(base_path, dataset, 'Evaluation/3D_Metrics/JSD/Bond')
    output_dir = os.path.join(base_path, dataset, 'Evaluation/3D_Metrics/JSD/results')
    os.makedirs(output_dir, exist_ok=True)
    
    # Result file paths
    result_file = os.path.join(output_dir, 'bond_length_jsd_results.txt')
    error_log = os.path.join(output_dir, 'bond_length_error_log.txt')
    ...
    for pair in file_pairs:
        try:
            ...
            # Record the result
            bond_type = pair[1].replace('.txt', '')
            results.append(f"{bond_type}\t{jsd:.4f}\n")
...
if __name__ == "__main__":
    ...
    # Run the calculation
    calculate_JSD_bond_length(base_path, dataset)
